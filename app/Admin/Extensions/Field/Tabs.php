...
class Tabs extends Field
{
    ...
    protected function buildRelatedForms()
    {
        ...
        if ($values = old($this->column)) {
            foreach ($values as $key => $data) {
                ...
                $tabKey = $data[$this->tabKey];
                $forms[$tabKey] = $this->buildNestedForm($this->column, $this->builder, $key)
                    ->fill($data);
            }
        } else {
    ...
    protected function setupScriptForTabView($templateScript)
    {
        ...
        $required = false;
        if(in_array('required', explode('|', $this->rules))) {
            $required = true;
        }
        ...
        $script = <<<EOT
            ...
            $('{$summernoteField}', $('#has-many-{$this->column} > .tab-content .tab-pane.active')).summernote({height: 300, callbacks: { onImageUpload: preprocesImage}});
            ...
            if($required && !\$('#has-many-{$this->column} ul.nav-tabs li').length) {
                $('#has-many-{$this->column} > .header .add').trigger('click');
            }

            $('#has-many-{$this->column} > .nav').off('click', 'li').on('click', 'li:not(.active)', function() {
                var target = $(this).find('> a').attr('href');
                $('{$summernoteField}',  $(target)).summernote({height: 300, callbacks: { onImageUpload: preprocesImage}});
            });
